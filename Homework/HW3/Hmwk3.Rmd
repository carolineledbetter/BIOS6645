...
1.   
  a.
    iii.Blah blah blah.
    iv. I chose a thresholf of `r round(youd_gbm, 2)` based on the RUC curve 
        using youden's method with `r best.iter` trees.   
...
pred$glmtest <- predict(glm1, data = TBI_test, type = 'response')
pred$glmtest2 <- ifelse(pred$glmtest <= youd_gbm, F, T)
acc$glmtest <- sum(pred$glmtest2 == TBI_test$d.unfav)/nrow(TBI_test)
...
```
  ...
  b.  
    ...
    ii. For the training data set (the US data) the predictive accuracy is 
        ...
        (the international data)
        it is `r round(acc$glmtest, 2)`.%  
    iii.No, the predictive accurcy on the test set is similar to the training 
        set. Both sets are also have predicition accuracies similar to that of 
        the test set using the gbm model. This indicates that this might be 
        approximately the best you can do with the information you have.  
